...
struct ChunkKey {
    int64_t x = 0, y = 0;
    bool operator<(const ChunkKey& other) const { return x != other.x ? x < other.x : y < other.y; }
...
enum class DeviceType { Power, Camera, Door };
struct Device {
    ChunkKey key;
...
public:
    ...
    ChunkChanges& Change(const ChunkKey& key) { return changes_[key]; }
    ...
    bool HasSaveWarning() const { return saveWarning_; }
