...
PostProcessor::~PostProcessor() {
    ...
    if (downsampleProgram_) glDeleteProgram(downsampleProgram_);
...
bool PostProcessor::MakeTarget(Target& target, int width, int height) {
    ...
    if (!target.framebuffer || !target.texture) return false;
...
void PostProcessor::Resize(int width, int height) {
    ...
    if (ready_ && width_ == width && height_ == height) return;
    ReleaseTargets();
    width_ = width; height_ = height;
    halfWidth_ = std::max(1, (width + 1) / 2);
    halfHeight_ = std::max(1, (height + 1) / 2);
    if (!downsampleProgram_ || !blurProgram_ || !compositeProgram_ || !vao_) return;
    GLint maxSize = 0;
    ...
    for (Target* target : { &bloom_[0], &bloom_[1], &softScene_[0], &softScene_[1] }) {
        if (success) success = MakeTarget(*target, halfWidth_, halfHeight_);
    }
...
void PostProcessor::Present() {
    ...
    const float edgeStrength = settings.edgeBlur ? std::clamp(settings.edgeBlurStrength, 0.f, 1.f) : 0;
    if (bloomStrength > 0) {
    ...
    glActiveTexture(GL_TEXTURE1); glBindTexture(GL_TEXTURE_2D, bloomStrength > 0 ? bloom_[0].texture : scene_.texture);
    glActiveTexture(GL_TEXTURE2); glBindTexture(GL_TEXTURE_2D, edgeStrength > 0 ? softScene_[0].texture : scene_.texture);
    ...
    glUniform2f(composite_.edgeRange, edgeStart, std::clamp(settings.edgeBlurEnd, edgeStart + .01f, 1.5f));
    glUniform1f(composite_.vignetteStrength, settings.vignette ? std::clamp(settings.vignetteStrength, 0.f, .95f) : 0);
    const float vignetteStart = std::clamp(settings.vignetteStart, 0.f, 1.39f);
    glUniform2f(composite_.vignetteRange, vignetteStart, std::clamp(settings.vignetteEnd, vignetteStart + .01f, 1.5f));
    DrawTriangle();
